...
@section('content')
    ...
    <form method="POST" action="{{ route('teacher.announcements.update', [$course, $announcement]) }}" class="mt-6 bg-white rounded-2xl shadow-sm border border-gray-100 p-5 space-y-4">
        ...
        <div>
            ...
            <input name="title" value="{{ old('title', $announcement->title) }}" class="mt-2 w-full rounded-xl border-gray-200 focus:border-violet-500 focus:ring-violet-500" />
            @error('title')
        ...
        <div>
            ...
            <textarea name="message" rows="6" class="mt-2 w-full rounded-xl border-gray-200 focus:border-violet-500 focus:ring-violet-500">{{ old('message', $announcement->message) }}</textarea>
            @error('message')
                <div class="text-xs text-red-600 mt-1">{{ $message }}</div>
            @enderror
        </div>
