...
@section('content')
    ...
    <form method="POST" action="{{ route('teacher.assignments.store', $course) }}" class="mt-6 bg-white rounded-2xl shadow-sm border border-gray-100 p-5 space-y-4">
        ...
        <div>
            ...
            <textarea name="description" rows="4" class="mt-2 w-full rounded-xl border-gray-200 focus:border-violet-500 focus:ring-violet-500">{{ old('description') }}</textarea>
            @error('description')
                <div class="text-xs text-red-600 mt-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="max_score" value="{{ old('max_score', 100) }}" class="mt-2 w-full rounded-xl border-gray-200 focus:border-violet-500 focus:ring-violet-500" />
                @error('max_score')
                    <div class="text-xs text-red-600 mt-1">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-2">
            ...
            <button type="submit" class="inline-flex items-center justify-center h-11 px-5 rounded-xl bg-violet-600 text-white text-sm font-semibold hover:bg-violet-700">Create</button>
        </div>
    </form>
@endsection
