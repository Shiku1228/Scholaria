...
@section('content')
    @php
        $maxScore = $exam->max_score ?? $attempt->max_score ?? $attempt->answers->sum(fn ($answer) => $answer->question?->points ?? 0);
        ...
        $hasManualReview = $attempt->answers->contains(fn ($answer) => $answer->question && !$answer->question->canAutoGrade());
        $student = $attempt->student;
        $course = $exam->course;
        $timeTakenSeconds = $attempt->started_at
            ? (int) ($attempt->submitted_at?->diffInSeconds($attempt->started_at) ?? now()->diffInSeconds($attempt->started_at))
            : 0;

        if ($timeTakenSeconds >= 3600) {
            $hours = intdiv($timeTakenSeconds, 3600);
            $minutes = intdiv($timeTakenSeconds % 3600, 60);
            $timeTakenLabel = $hours . ' hr' . ($hours !== 1 ? 's' : '') . ($minutes > 0 ? ' ' . $minutes . ' min' : '');
        } elseif ($timeTakenSeconds >= 60) {
            $minutes = intdiv($timeTakenSeconds, 60);
            $seconds = $timeTakenSeconds % 60;
            $timeTakenLabel = $minutes . ' min' . ($seconds > 0 ? ' ' . $seconds . ' sec' : '');
        } else {
            $timeTakenLabel = $timeTakenSeconds . ' sec';
        }
    @endphp
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            ...
            <div class="text-sm font-semibold text-slate-900 truncate">{{ $student->name ?? 'Unknown Student' }}</div>
            <div class="text-xs text-slate-500 mt-1">{{ $student->student_number ?? 'No student number' }}</div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            <div class="text-xs text-slate-500 mb-1">Email</div>
            <div class="text-sm font-semibold text-slate-900 break-all">{{ $student->email ?? 'No email' }}</div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            <div class="text-xs text-slate-500 mb-1">Course</div>
            <div class="text-sm font-semibold text-slate-900">{{ $course->course_number ?? 'No course number' }}</div>
            <div class="text-xs text-slate-500 mt-1">{{ $course->title ?? 'Untitled Course' }}</div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            ...
            <div class="text-sm font-semibold text-slate-900">{{ $attempt->submitted_at?->format('M d, Y g:i A') ?? 'Not submitted yet' }}</div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            <div class="text-xs text-slate-500 mb-1">Time Taken</div>
            <div class="text-sm font-semibold text-slate-900">{{ $timeTakenLabel }}</div>
            <div class="text-xs text-slate-500 mt-1">
                @if($isGraded)
                    Graded
                    @if($attempt->graded_at)
                        {{ $attempt->graded_at->format('M d g:i A') }}
                    @endif
    ...
    <form method="POST" action="{{ route('teacher.exams.attempts.grade', [$exam, $attempt]) }}">
        @csrf
            ...
            <div class="px-5 py-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button type="button" id="reset-auto-btn"
                        ...
                        <i data-lucide="rotate-ccw" class="h-3 w-3 mr-1.5"></i>Reset Scores
                    </button>
                </div>
            </div>
            ...
            <div class="divide-y divide-slate-100">
                @forelse($attempt->answers as $index => $answer)
                    @php
                        ...
                        if (!$question) {
                            $badge = ['label' => 'Question Missing', 'cls' => 'bg-slate-100 text-slate-700'];
                        ...
                        } elseif ($pointsEarned === $maxPoints && $maxPoints > 0) {
                    ...
                    <div class="p-5" data-question-row data-auto-score="{{ $autoScore }}" data-max-pts="{{ $maxPoints }}">
                            ...
                            <div class="flex items-start gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <div class="flex flex-wrap items-center gap-2 mt-1.5">
                                        ...
                                        <span class="text-xs text-slate-500 font-medium">{{ $maxPoints }} pts possible</span>
                                ...
                                @foreach($question->options as $key => $option)
                                    @php
                                        ...
                                        if ($isSelected && $isCorrect) {
                                            ...
                                            $optionClass = 'bg-emerald-50/60 border-emerald-200 text-emerald-700';
                                        }
                                    @endphp
                                    <div class="flex items-center justify-between p-2.5 border rounded-lg text-sm {{ $optionClass }}">
                        ...
                        @elseif($question?->isTrueFalse())
                            <div class="flex gap-3 mb-4">
                                @foreach(['true', 'false'] as $value)
                                    @php
                                        $isSelected = strtolower((string) $answer->answer) === $value;
                                        $isCorrect = strtolower((string) $question->correct_answer) === $value;
                                        $optionClass = 'bg-slate-50 border-slate-200 text-slate-700';
                                        if ($isSelected && $isCorrect) {
                                            $optionClass = 'bg-emerald-50 border-emerald-300 text-emerald-800 font-semibold';
                                        } elseif ($isSelected && !$isCorrect) {
                                    ...
                                    @endphp
                                    <div class="flex items-center gap-2 px-4 py-2 border rounded-lg text-sm {{ $optionClass }}">
                        ...
                        @else
                            <div class="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-4">
                                <div class="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1.5">Student Answer</div>
                            ...
                            <div class="flex items-center gap-4 flex-wrap">
                                <div class="flex items-center gap-2">
                                    ...
                                    <input type="number"
                                        ...
                                        step="1"
                                        data-score-input
                                        class="w-20 rounded-lg border border-slate-300 bg-white text-sm font-semibold text-slate-900 px-3 py-1.5 text-center focus:border-[#0b2d6b] focus:ring-[#0b2d6b] @error("scores.{$question?->id}") border-red-400 @enderror"
                                        {{ !$question ? 'disabled' : '' }}
                                    >
                                    <span class="text-xs text-slate-500 font-medium">/ {{ $maxPoints }} pts</span>
                                </div>

                                @if($maxPoints > 0 && $question)
                                    <div class="flex items-center gap-1.5">
                                        <button type="button" onclick="setScore(this, {{ $maxPoints }})"
                                        ...
                                        @if($maxPoints > 1)
                                            <button type="button" onclick="setScore(this, {{ (int) round($maxPoints / 2) }})"
                                                ...
                                                Half ({{ (int) round($maxPoints / 2) }})
                                            </button>
                                        @endif
                                        <button type="button" onclick="setScore(this, 0)"
            ...
            <div class="bg-white border border-slate-200 rounded-2xl shadow-xl px-5 py-4 flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button type="submit"
                        ...
                        <i data-lucide="save" class="h-4 w-4 mr-2"></i>Save Grading
                    </button>
                </div>
            </div>
        </div>
    ...
    <script>
        function setScore(button, value) {
        ...
        document.querySelectorAll('[data-score-input]').forEach(function(input) {
            input.addEventListener('input', function() {
                ...
                const maxPoints = parseFloat(row?.dataset.maxPts ?? 0);
        ...
        document.getElementById('reset-auto-btn')?.addEventListener('click', function() {
            if (!confirm('Reset all scores to the saved values for this attempt?')) {
                return;
            }
