...
@section('content')
    ...
    <div class="mt-6 bg-white rounded-2xl shadow-sm border border-gray-100" data-tabs-root>
        ...
        <div data-tab-panel="announcements" class="p-4 sm:p-6 hidden">
            <div class="flex items-start sm:items-center justify-between gap-2 mb-4">
                <div>
                ...
                <div class="flex flex-wrap items-center gap-2">
                    <button
                        type="button"
                        id="quickCreateAnnouncementToggle"
                        class="inline-flex items-center justify-center h-10 px-4 rounded-xl bg-[#eaf0fb] text-[#0b2d6b] text-sm font-semibold hover:bg-[#dbe7ff]"
                        aria-expanded="false"
                        aria-controls="quickCreateAnnouncementPanel"
                    >
            ...
            <div id="quickCreateAnnouncementPanel" class="hidden mb-6 rounded-xl border border-slate-200 bg-white p-4">
                <div class="flex items-start justify-between gap-4 mb-3">
                    <div>
                        <div class="text-sm font-semibold text-slate-900">Quick Create Announcement</div>
                        <div class="text-xs text-slate-500">Post a new announcement without leaving this page.</div>
                    </div>
                    <button
                        type="button"
                        id="quickCreateAnnouncementCancel"
                        class="inline-flex items-center justify-center h-9 w-9 rounded-lg border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"
                        aria-label="Cancel quick create"
                    >
                        ✕
                    </button>
                </div>

                <form method="POST" action="{{ route('teacher.announcements.store', $course) }}" class="space-y-3">
                    @csrf

                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1" for="quickCreateTitle">Title</label>
                        <input
                            id="quickCreateTitle"
                            type="text"
                            name="title"
                            value="{{ old('title') }}"
                            placeholder="e.g. Week 3: Assignment released"
                            class="h-10 w-full rounded-xl border-gray-200 text-sm focus:border-[#0b2d6b] focus:ring-[#0b2d6b]"
                            required
                        >
                        @error('title')
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1" for="quickCreateMessage">Message</label>
                        <textarea
                            id="quickCreateMessage"
                            name="message"
                            rows="3"
                            placeholder="Write the announcement message..."
                            class="w-full rounded-xl border-gray-200 text-sm focus:border-[#0b2d6b] focus:ring-[#0b2d6b]"
                            required
                    ...
                    <div class="flex items-center justify-end gap-2 pt-1">
                        <button type="button" id="quickCreateAnnouncementCloseSecondary" class="inline-flex items-center justify-center h-10 px-4 rounded-xl border border-slate-200 bg-white text-sm font-semibold text-slate-700 hover:bg-slate-50">
                            Cancel
                        </button>
                        <button type="submit" class="inline-flex items-center justify-center h-10 px-4 rounded-xl bg-[#0b2d6b] text-white text-sm font-semibold hover:bg-[#0a275c]">
                            Post
                        </button>
                    </div>
                </form>
    ...
    <script>
        (function () {
            ...
            // Quick Create Announcement toggle
            const quickToggle = document.getElementById('quickCreateAnnouncementToggle');
            const quickPanel = document.getElementById('quickCreateAnnouncementPanel');
            const quickCancel1 = document.getElementById('quickCreateAnnouncementCancel');
            const quickCancel2 = document.getElementById('quickCreateAnnouncementCloseSecondary');
            ...
            if (quickToggle && quickPanel) {
                quickToggle.addEventListener('click', function () {
                    ...
                    setQuickPanel(isHidden);
                });
            }
            if (quickCancel1) quickCancel1.addEventListener('click', function () { setQuickPanel(false); });
            if (quickCancel2) quickCancel2.addEventListener('click', function () { setQuickPanel(false); });
