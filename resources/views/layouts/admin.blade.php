...
<div class="min-h-screen flex">
    ...
    <aside id="sidebar" class="w-64 flex flex-col bg-white border-r border-gray-200">
        <div class="h-16 flex items-center justify-between px-6 border-b border-gray-200">
            ...
            <button id="sidebarClose" type="button" class="min-[992px]:hidden p-2 rounded-md hover:bg-gray-100">
                ✕
            </button>
        </div>
    ...
    <div class="flex-1 flex flex-col min-w-0">
        <header class="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-4 sm:px-6">
            ...
            <form method="POST" action="{{ route('logout') }}" class="flex items-center">
                @csrf
                <button type="submit" class="inline-flex items-center justify-center h-9 px-4 rounded-lg border border-gray-200 text-sm font-medium text-gray-700 hover:bg-gray-50">
                    Logout
                </button>
            </form>
        </header>
