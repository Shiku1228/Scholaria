    ...
    <tbody class="divide-y divide-slate-200">
        ...
        <tr class="text-slate-700 bg-slate-50/60 hover:bg-slate-100/60 transition-colors">
            ...
            <td class="py-4 px-6 text-center">
                ...
                @else
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $exam->is_overdue ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700' }}">
                        <i data-lucide="calendar" class="h-3 w-3 mr-1"></i>
                        {{ $exam->is_overdue ? 'Missed' : 'Scheduled' }}
                    </span>
                @endif
            </td>
            <td class="py-4 px-6">
                @if($exam->score !== null)
                    <div class="text-sm font-medium text-slate-900">{{ $exam->score }}/{{ $exam->max_score ?? 'N/A' }}</div>
                @else
