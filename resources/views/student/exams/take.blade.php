...
@section('content')
    ...
    <div class="fixed top-0 left-0 right-0 bg-[#0b2d6b] text-white z-50 py-3 px-4 shadow-lg">
        <div class="max-w-4xl mx-auto flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <span class="font-mono text-lg" id="timer">--:--</span>
            </div>
            <div class="text-sm">
                {{ $exam->title }}
            </div>
    ...
    <div class="pt-16 max-w-3xl mx-auto px-4 py-6">
        <form method="POST" action="{{ route('student.exams.submit', $exam) }}" id="exam-form">
            ...
            <div class="mt-8 flex items-center justify-between">
                <div class="text-sm text-slate-500">
                    <span id="answered-count">0</span> of {{ $exam->questions->count() }} answered
                </div>
                <button type="submit" class="inline-flex items-center justify-center h-11 px-6 rounded-xl bg-green-600 text-white text-sm font-semibold hover:bg-green-700 shadow-sm" onclick="return confirm('Submit your exam? You cannot change answers after submission.')">
    ...
    <script>
        ...
        function updateAnsweredCount() {
            ...
            @foreach($exam->questions as $question)
                ...
                if (q{{ $question->id }}.length > 0 && (q{{ $question->id }}[0].value || q{{ $question->id }}[0].checked)) {
                    answered++;
                }
            @endforeach
