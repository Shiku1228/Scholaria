...
@section('content')
    ...
    @if($exam->isFaceToFace())
        {{-- Face-to-Face: info-only card, no answer form --}}
        @php
        ...
        <div class="bg-amber-50 border border-amber-200 rounded-xl p-6 mb-6 flex items-start gap-4">
            ...
            <div>
                ...
                @if($ftfPast)
                    <p class="text-sm text-amber-800">This face-to-face exam has already been conducted. Please contact your instructor if you need more information.</p>
                @else
                    <p class="text-sm text-amber-800">This exam will be conducted face-to-face. Please follow your teacher's instructions and be present on the scheduled date.</p>
                @endif
            </div>
        </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                            ...
                            @if($exam->exam_date)
                                <div>
                                    ...
                                    <div class="font-semibold text-slate-900 mt-0.5">{{ $exam->exam_date->format('M j, Y g:i A') }}</div>
                                </div>
                            @endif
                            @if($exam->location)
                                <div>
                        ...
                        @if($exam->show_results && !($exam->feedback_type === 'delayed' && !$exam->results_released))
                            <div class="space-y-4">
                                ...
                                @foreach($selectedAttempt->answers as $index => $answer)
                                    @php 
                                        $q = $answer->question; 
                                        $answerScore = $answer->points_earned;
                                        $isCorrect = $answerScore !== null && $answerScore > 0;
                                        $isGraded = $answerScore !== null;
                                    @endphp
                                    <div class="border border-slate-200 rounded-lg p-4 space-y-3">
                                        <div class="flex items-start justify-between gap-4">
                                            ...
                                            <span class="text-xs font-semibold px-2 py-0.5 rounded-full {{ $isGraded ? ($isCorrect ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700') : 'bg-slate-100 text-slate-600' }}">
                                                {{ $isGraded ? $answerScore . ' / ' . $q->points . ' pts' : 'Awaiting Grading' }}
                                            </span>
                                        </div>
